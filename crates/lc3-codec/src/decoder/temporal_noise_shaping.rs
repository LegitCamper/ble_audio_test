// Local modification: replace per-frame sin with the equivalent fixed-domain table.
use super::fast_math_tables::REFLECTION_COEFFICIENTS;
use crate::{
...
pub fn apply_temporal_noise_shaping(
    ...
    // Reflection indices have a fixed 0..=16 domain, so avoid evaluating sin for every frame.
    for (rc_q_item, rc_i_item) in rc_quant.iter_mut().zip(reflect_coef_ints) {
        *rc_q_item = REFLECTION_COEFFICIENTS[*rc_i_item];
    }
...
mod tests {
    ...
    #[test]
    fn reflection_table_should_match_original_math_for_every_index() {
        for (index, coefficient) in REFLECTION_COEFFICIENTS.iter().enumerate() {
            let expected = if index == 0 {
                0.0
            } else {
                num_traits::real::Real::sin((core::f64::consts::PI / 17.0) as f32 * (index as i32 - 8) as f32)
            };
            assert_eq!(coefficient.to_bits(), expected.to_bits(), "reflection index={index}");
        }
    }
