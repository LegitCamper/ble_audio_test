// Local modification: integer spectral-pair bit counts, with coverage for all frame sizes.
use super::{
...
pub fn read(
    ...
    // last non-zero tuple
    let spectral_pairs = ne / 2;
    let lastnz_num_bits = (usize::BITS - (spectral_pairs - 1).leading_zeros()) as usize;
...
mod tests {
    ...
    #[test]
    fn last_nonzero_tuple_should_use_the_expected_bits_for_every_frame_size() {
        for ne in [60, 80, 120, 160, 180, 240, 300, 320, 400] {
            let bits = ((ne / 2) as f32).log2().ceil() as usize;
            for encoded_tuple in 0..(1_usize << bits) {
                // A narrow-band header has no bandwidth bits; lastnz is first at the tail.
                let mut buf = [0; 16];
                buf[15] = encoded_tuple as u8;
                let mut reader = BufferReader::new();
                let result = read(&buf, &mut reader, 0, ne);
                let lastnz = (encoded_tuple + 1) * 2;
                if lastnz <= ne {
                    assert_eq!(result.unwrap().lastnz, lastnz);
                } else {
                    assert!(
                        matches!(result, Err(SideInfoError::LastNonZeroTupleGreaterThanYLen(actual, limit)) if actual == lastnz && limit == ne)
                    );
                }
            }
        }
