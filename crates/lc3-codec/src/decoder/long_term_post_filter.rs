// Local modification: exact integer gain/pitch math, with exhaustive domain tests.
...
impl<'a> LongTermPostFilter<'a> {
    ...
    fn compute_gains_params(&self, nbits: usize) -> (Scaler, usize) {
        ...
        let t_nbits = match self.config.n_ms {
            // round(nbits * 10 / 7.5), with integer inputs and no halfway cases.
            FrameDuration::SevenPointFiveMs => (nbits * 4 + 1) / 3,
    ...
    fn compute_filter_parameters(&self, info: &LongTermPostFilterInfo) -> (usize, usize) {
        if !info.is_active {
            ...
            let (pitch_int, pitch_fr) = if pitch_index >= 440 {
                ...
                let pitch_fr = 2 * pitch_index - 4 * pitch_int - 252;
                (pitch_int, pitch_fr)
            } else {
                let pitch_int = pitch_index / 4 + 32;
                let pitch_fr = pitch_index + 128 - 4 * pitch_int;
                (pitch_int, pitch_fr)
            };

            let pitch_quarters = pitch_int * 4 + pitch_fr;
            // 8000 / 12800 = 5 / 8; adding four rounds nonnegative eighths up.
            let sample_rate_multiple = (self.config.fs + 7_999) / 8_000;
            let p_up = (pitch_quarters * 5 * sample_rate_multiple + 4) / 8;
            let p_int = p_up / 4;
...
mod tests {
    ...
    #[test]
    fn pitch_and_gain_should_match_original_math_across_the_supported_domain() {
        for frequency in [
            SamplingFrequency::Hz8000,
            SamplingFrequency::Hz16000,
            SamplingFrequency::Hz24000,
            SamplingFrequency::Hz32000,
            SamplingFrequency::Hz44100,
            SamplingFrequency::Hz48000,
        ] {
            for duration in [FrameDuration::SevenPointFiveMs, FrameDuration::TenMs] {
                let config = Lc3Config::new(frequency, duration);
                let mut scratch = std::vec![0.0; LongTermPostFilter::calc_working_buffer_length(&config)];
                let (filter, _) = LongTermPostFilter::new(config, &mut scratch);
                for pitch_index in 0..512 {
                    let pitch = if pitch_index >= 440 {
                        (pitch_index - 283) as f64
                    } else if pitch_index >= 380 {
                        (pitch_index / 2 - 63) as f64 + (pitch_index % 2) as f64 / 2.0
                    } else {
                        (pitch_index / 4 + 32) as f64 + (pitch_index % 4) as f64 / 4.0
                    };
                    let p_up = (pitch * (8000.0 * (config.fs as f64 / 8000.0).ceil() / 12800.0) * 4.0 + 0.5) as usize;
                    let info = LongTermPostFilterInfo::new(true, true, pitch_index);
                    assert_eq!(filter.compute_filter_parameters(&info), (p_up / 4, p_up % 4));
                    let inactive = LongTermPostFilterInfo::new(false, true, pitch_index);
                    assert_eq!(filter.compute_filter_parameters(&inactive), (0, 0));
                }
                for nbits in 0..=3200 {
                    let scaled = match duration {
                        FrameDuration::SevenPointFiveMs => (nbits as f64 * 10.0 / 7.5).round() as usize,
                        FrameDuration::TenMs => nbits,
                    };
                    let expected = match scaled.saturating_sub(config.fs_ind * 80) {
                        0..320 => (0.4, 0),
                        320..400 => (0.35, 1),
                        400..480 => (0.3, 2),
                        480..560 => (0.25, 3),
                    ...
                    assert_eq!(filter.compute_gains_params(nbits), expected);
                }
            }
        }
    }
