// Local modification: integer arithmetic-coder bit counts, with equivalence tests.
use super::{
...
fn calc_num_residual_bits(
    ...
    // The arithmetic decoder normalizes its nonzero range to at least 0x10000.
    let range_log2 = (u32::BITS - 1 - st.ac_range.leading_zeros()) as usize;
    let nbits_ari = (reader.get_head_byte_cursor() + 1 - 3) * 8 + 25 - range_log2;
...
mod tests {
    ...
    #[test]
    fn residual_bit_budget_should_match_original_math_at_range_boundaries() {
        let reader = BufferReader::new_at(10, 64);
        for power in 16..24 {
            for range in [(1_u32 << power) - 1, 1 << power, (1 << power) + 1] {
                let state = ArithmeticDecoderState {
                    ac_low: 0,
                    ac_range: range,
                };
                let expected = 1200 - (64 - 8) - ((10 + 1 - 3) * 8 + 25 - (range as f64).log2().floor() as usize);
                assert_eq!(calc_num_residual_bits(&reader, &state, 1200).unwrap(), expected);
                assert!(matches!(
                    calc_num_residual_bits(&reader, &state, 8),
                    Err(ArithmeticDecodeError::NegativeResidualNumBits)
                ));
            }
        }
